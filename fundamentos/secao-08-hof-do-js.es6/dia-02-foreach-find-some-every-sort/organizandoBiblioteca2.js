...
//Crie um array com strings no formato NOME_DO_LIVRO - GÊNERO_DO_LIVRO - NOME_DA_PESSOA_AUTORA
  const formatedBookNames = (param) => {
    // escreva seu código aqui
    return param.map((param) => `${param.name} - ${param.genre} - ${param.author.name}`);
  }
  ...
  const formatedAuthorNamesBirth = (param) => {
    // escreva seu código aqui
    return param.map((param) => `${param.author.name} - ${param.author.birthYear}`);
  }
  ...
  const nameAndAge = (param) => {
    // escreva seu código aqui
    return param
    .map((objeto) => ({
        author: objeto.author.name,
        age: objeto.author.birthYear,
    })) 
